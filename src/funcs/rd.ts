...
import { db } from "@/db";
import { rdDepartments, rdResearchAreas, rdFocusAreas, rdFunders, rdConsultancy, rdCommittee, rdProjects, rdScholars, rdCoordinatorMessage, rdMotto, rdPublications, rdPublicationStats, rdMous } from "@/db/schema";
...
export const updateDepartment = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { id: number; name: string } }) => {
...
export const addArea = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { deptId: number; area: string } }) => {
...
export const updateArea = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { id: number; area: string } }) => {
    const { id, ...update } = data;
    return await db.update(rdResearchAreas).set(update).where(eq(rdResearchAreas.id, id)).returning();
  });
...
export const addFocusArea = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { title: string; description: string; icon: string } }) => {
    return await db.insert(rdFocusAreas).values(data).returning();
  });

export const updateFocusArea = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { id: number; title?: string; description?: string; icon?: string } }) => {
    const { id, ...update } = data;
...
export const updateConsultancy = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { id: number; name?: string; description?: string } }) => {
    const { id, ...update } = data;
...
export const addMember = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { name: string; role: string; detail: string } }) => {
    return await db.insert(rdCommittee).values(data).returning();
  });

export const updateMember = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { id: number; name?: string; role?: string; detail?: string } }) => {
    const { id, ...update } = data;
...
export const updateProject = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { id: number; [key: string]: any } }) => {
    const { id, ...update } = data;
...
export const updateCoordinatorMessage = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: any }) => {
    const { id, ...update } = data;
    return await db.update(rdCoordinatorMessage).set(update).where(eq(rdCoordinatorMessage.id, id)).returning();
  });
...
export const updateMotto = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { id: number; text: string } }) => {
    return await db.update(rdMotto).set({ text: data.text }).where(eq(rdMotto.id, data.id)).returning();
  });
...
export const updatePublicationStat = createServerFn({ method: "POST" })
  .handler(async ({ data }: { data: { id: number; value: number } }) => {
    return await db.update(rdPublicationStats).set({ value: data.value }).where(eq(rdPublicationStats.id, data.id)).returning();
  });
