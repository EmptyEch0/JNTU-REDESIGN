...
export const Route = createFileRoute("/administration/principal")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Principal's message and profile of JNTU-GV College of Engineering Vizianagaram." },
    ],
  }),
...
function PrincipalPage() {
  ...
  if (isPrincipalLoading || !principal) return <div className="min-h-screen grid place-items-center"><div className="animate-spin h-8 w-8 border-4 border-primary border-t-transparent rounded-full" /></div>;
          ...
          <div className="mt-20 pt-16 border-t border-border max-w-6xl mx-auto">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">The Principal’s Office plays a key role in supporting the academic activities and the overall management of CEV.</p>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-muted/50 border-b border-border">
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground">S.No</th>
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground">Name of the Employee</th>
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground">Section</th>
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground">Designation</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {isStaffLoading ? (
                    <tr><td colSpan={4} className="px-6 py-12 text-center text-muted-foreground animate-pulse">Loading staff records...</td></tr>
                    ...
                    <tr key={member.id} className="group hover:bg-primary/[0.02] transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm font-bold text-ink group-hover:text-primary transition-colors">{member.name}</td>
                      <td className="px-6 py-4 text-sm text-muted-foreground font-medium">{member.section}</td>
