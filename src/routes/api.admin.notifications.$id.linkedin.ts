...
import { db } from "@/db";
import { notices, socialConnections, socialPosts } from "@/db/schema";
...
export const Route = createFileRoute("/api/admin/notifications/$id/linkedin")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        ...
        console.log("Publish request received for notice item:", id);
        ...
        try {
          ...
          const commentary = body.caption?.trim() || notice.title;
          ...
          if (notice.url) {
            ...
            if (isImage) {
              console.log("Calling LinkedIn API: Registering notice image...");
              ...
              console.log("LinkedIn API response received for notice image registration. Status:", registerRes.status);
              ...
              console.log("Calling LinkedIn API: Uploading notice image binary...");
              const { buffer, mimeType } = await getImageBuffer(notice.url);
          ...
          console.log("Calling LinkedIn API: Creating notice post...");
          ...
          console.log("LinkedIn API response received for notice creation. Status:", postRes.status);
          ...
          console.error("LinkedIn notice publish error:", {
            name: error?.name,
          ...
          // Try to update notices status in DB
          try {
            await db
              .update(notices)
              .set({
