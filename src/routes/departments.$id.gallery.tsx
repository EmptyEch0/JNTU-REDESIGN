    ...
    <div className="animate-in fade-in slide-in-from-bottom-6 duration-300 pb-20">
      ...
      <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
        {filteredImages.map((image: any) => (
          ...
          >
            {image.image_url ? (
              <img decoding="async" loading="lazy" src={getAssetUrl(image.image_url)} alt={image.title} className="w-full object-cover transition-transform duration-700 group-hover:scale-110" />
            ) : (
      ...
      {selectedImage && !isEditMode && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 animate-in fade-in duration-300" onClick={() => setSelectedImage(null)}>
          ...
          <div className="relative max-w-5xl w-full animate-in zoom-in duration-300" onClick={e => e.stopPropagation()}>
            <img decoding="async" loading="lazy" src={getAssetUrl(selectedImage.image_url)} className="w-full h-auto max-h-[80vh] object-contain rounded-2xl shadow-2xl" alt="" />
