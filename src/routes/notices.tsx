...
function NoticesPage() {
  ...
  const handlePublishNotice = async (e: React.FormEvent) => {
    ...
    const tId = toast.loading("Publishing official notice...");
    try {
      const now = new Date();
      const dateStr = `${now.getDate().toString().padStart(2, "0")} ${now.toLocaleDateString("en-US", { month: "short" })} ${now.getFullYear()}`;
      ...
      toast.success("Notice published & push alert broadcast!", { id: tId });
      setShowAddModal(false);
      setNewNotice({ title: "", tag: "Academic", url: "" });
      router.invalidate();
    } catch (err: any) {
      toast.error(err.message || "Failed to publish notice", { id: tId });
    } finally {
  ...
  const handleDeleteNotice = async (id: number, title: string) => {
    ...
    try {
      ...
      toast.success("Notice deleted successfully", { id: tId });
      router.invalidate();
    } catch {
  ...
  return (
    <div className="bg-slate-50/60 dark:bg-slate-950 min-h-screen">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto justify-end">
              {isAdmin && (
                <button
                  ...
                  className="px-4 py-2.5 bg-[#0F4C81] hover:bg-[#0D3F6D] text-white rounded-2xl text-xs font-bold transition shadow-md shadow-[#0F4C81]/20 flex items-center gap-2 cursor-pointer shrink-0 animate-pulse hover:animate-none"
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0 self-end sm:self-center pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800/60 w-full sm:w-auto justify-end">
                    ...
                    {isEditMode && (
                      <button
                        ...
                        className="inline-flex items-center p-2 rounded-xl bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 hover:bg-rose-600 hover:text-white border border-rose-200/80 dark:border-rose-900/60 text-xs font-bold transition-all shadow-xs cursor-pointer active:scale-95"
                        title="Delete Notice"
                      >
      ...
      {/* Interactive Publish New Notice Modal for Admins */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-scale-reveal">
            <div className="p-5 sm:p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4 bg-slate-50/70 dark:bg-slate-850/60">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-[#0F4C81] text-white flex items-center justify-center shadow-md">
                  <Bell className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white leading-snug">
                    Publish Official Circular / Notice
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Will appear live across the website & send instant push notifications
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handlePublishNotice} className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-4">
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Smart India Hackathon (SIH) 2026 — Internal Hackathon Registration"
                  value={newNotice.title}
                  onChange={(e) => setNewNotice({ ...newNotice, title: e.target.value })}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-[#0F4C81]/30 transition"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-[#0F4C81]/30 cursor-pointer"
                >
              ...
              <FileUploadDropzone
                label="Official Document / Picture Attachment (PDF, Images, Docs, Excel, Zip)"
                sublabel="Drag & drop any attachment (PDF, JPG, PNG, DOCX, XLSX, ZIP up to 50MB) or paste Google Form / URL link"
                value={newNotice.url}
                onChange={(uploadedPath) => setNewNotice({ ...newNotice, url: uploadedPath })}
                module="notices"
                category={newNotice.tag}
                fileNamePrefix={newNotice.title || "circular"}
              />
              ...
              <div className="pt-4 flex items-center justify-end gap-3 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-[#0F4C81] hover:bg-[#0D3F6D] text-white rounded-xl text-xs font-bold transition shadow-md shadow-[#0F4C81]/25 flex items-center gap-2 cursor-pointer disabled:opacity-50"
                >
                  <Plus className="w-4 h-4" />
