...
import { 
  ...
  getRecruiters, addRecruiter, deleteRecruiter,
...
function TrainingPage() {
  ...
  const { data: tpoData, isLoading: isLoadingTPO } = useQuery({ queryKey: ['tpo'], queryFn: () => getTPO() });
  const { data: goals, isLoading: isLoadingGoals } = useQuery({ queryKey: ['goals'], queryFn: () => getGoals() });
  const { data: recruiters, isLoading: isLoadingRecruiters } = useQuery({ queryKey: ['recruiters'], queryFn: () => getRecruiters() });
  const { data: staff, isLoading: isLoadingStaff } = useQuery({ queryKey: ['staff'], queryFn: () => getStaff() });

  // Initialize editedTPO when data arrives
  useEffect(() => { if (tpoData) setEditedTPO(null); }, [tpoData]);
  ...
  const hasUnsavedChanges = editedTPO !== null || Object.keys(editedGoals).length > 0 || Object.keys(editedStaff).length > 0;

  return (
    <>
      <PageHero eyebrow="Placements" title="Training & Placement Cell" subtitle="Fostering excellence through industry collaboration and student empowerment." image={placementsImg} />
      <SubNav items={PLACEMENTS_SUBNAV} />

      <section className="py-20 bg-white">
        <div className="container-narrow grid md:grid-cols-2 gap-16 items-start">
          <RevealOnScroll>
            <div className={`p-8 rounded-3xl border transition-all relative group ${isEditMode ? "bg-amber-50/50 border-amber-200" : "bg-card border-border shadow-[var(--shadow-elegant)]"}`}>
              <div className="flex flex-col items-center text-center">
                <div className="relative group/img">
                  <img src={editedTPO?.image ?? tpoData?.image} alt={tpoData?.name} className="w-48 h-48 rounded-2xl object-cover shadow-lg mb-6" />
                  {isEditMode && (
                    <input 
      ...
      <section className="py-20 bg-sand">
        <div className="container-narrow">
          <RevealOnScroll><SectionLabel eyebrow="Strategic Focus" title="Goals of the T&P Cell" align="center" /></RevealOnScroll>
          
          <div className="mt-12 grid gap-6">
            {goals?.map((goal, idx) => (
              <RevealOnScroll key={goal.id} delay={idx * 50}>
                <div className={`flex gap-6 p-6 rounded-2xl border transition-all relative group ${isEditMode ? "bg-white border-amber-200 shadow-sm" : "bg-card border-border"}`}>
                  ...
                  <div className="flex-1">
                    {isEditMode ? (
                      <textarea 
                        ...
                        onChange={e => setEditedGoals({...editedGoals, [goal.id]: {text: e.target.value}})}
                      />
                    ) : <p className="text-ink leading-relaxed">{goal.text}</p>}
                  </div>
                  {isEditMode && (
                    <button onClick={() => handleDeleteGoal(goal.id)} className="text-red-400 hover:text-red-600 self-start">
                      <Trash2 size={18} />
                    </button>
                  )}
                </div>
            ...
            {isEditMode && (
              <button onClick={handleAddGoal} className="flex items-center justify-center gap-2 p-4 border-2 border-dashed border-primary/30 rounded-2xl text-primary font-bold hover:bg-primary/5 transition-all">
      ...
      <section className="py-20 bg-white">
        <div className="container-narrow">
          <RevealOnScroll><SectionLabel eyebrow="Recruitment Partners" title="Major Recruiters" align="center" /></RevealOnScroll>
          <div className="mt-12 grid grid-cols-2 sm:grid-cols-4 gap-4">
            {recruiters?.map((rec) => (
              <div key={rec.id} className={`p-4 rounded-xl border text-center font-medium transition-all group relative ${isEditMode ? "bg-amber-50/50 border-amber-200" : "bg-sand border-transparent hover:border-primary hover:bg-white"}`}>
                {rec.name}
                {isEditMode && (
                  <button onClick={() => handleDeleteRecruiter(rec.id)} className="absolute -top-2 -right-2 bg-red-500 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                    <Trash2 size={12} />
                  </button>
                )}
              </div>
            ...
            {isEditMode && (
              <button onClick={handleAddRecruiter} className="p-4 rounded-xl border-2 border-dashed border-primary/30 text-primary flex items-center justify-center gap-2 hover:bg-primary/5">
                <Plus size={16} /> Add
              </button>
            )}
          </div>
      ...
      <section className="py-20 bg-sand">
        <div className="container-narrow">
          <RevealOnScroll><SectionLabel eyebrow="Our Team" title="Supporting Staff" align="center" /></RevealOnScroll>
          <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {staff?.map((s) => (
              <div key={s.id} className={`p-6 rounded-2xl border transition-all relative group ${isEditMode ? "bg-white border-amber-200 shadow-sm" : "bg-card border-border"}`}>
                    ...
                    {isEditMode ? (
                      <div className="space-y-2">
                        <input 
                          ...
                          onChange={e => setEditedStaff({...editedStaff, [s.id]: {...editedStaff[s.id], name: e.target.value}})}
                        />
                        <input 
                          ...
                          onChange={e => setEditedStaff({...editedStaff, [s.id]: {...editedStaff[s.id], role: e.target.value}})}
                        />
                      </div>
                    ) : (
                      <>
            ...
            {isEditMode && (
              <button onClick={handleAddStaff} className="p-6 rounded-2xl border-2 border-dashed border-primary/30 text-primary flex items-center justify-center gap-2 hover:bg-primary/5 h-full min-h-[80px]">
