...
export const Route = createFileRoute("/rd-cell/mous")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Memoranda of Understanding signed with industry and research partners.",
      },
    ],
  }),
  component: MOUsPage,
  ...
  return (
    <>
      <PageHero
        eyebrow="R&D Cell"
        title="MOUs"
        subtitle="Industry, research and innovation partnerships that power our work."
        image={labImg}
      />
      ...
      <section className="py-20 container-narrow">
        <div className="flex justify-between items-end mb-10">
          <RevealOnScroll>
            <SectionLabel eyebrow="Department-wise" title="Active partnerships" />
          </RevealOnScroll>
          {isEditMode && (
            <button
              onClick={() => handleAddMou("department")}
              className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-full font-bold text-sm"
            >
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {departmentMous.map((m: any, i: number) => (
            <RevealOnScroll key={m.id} delay={i * 100}>
              <div
                className={`p-7 rounded-2xl bg-card border transition-all h-full relative group ${isEditMode ? "border-primary/30" : "border-border hover-lift"}`}
              >
                {isEditMode && (
                  <button
                    onClick={async () => {
                      if (confirm("Delete MOU?")) {
                        await deleteMou({ data: { id: m.id } });
                        queryClient.invalidateQueries({ queryKey: ["rdMous"] });
                      }
                    }}
                ...
                {isEditMode ? (
                  <div className="space-y-4 mt-2">
                    <input
                      className="text-display text-2xl text-ink w-full bg-primary/5 p-1 rounded"
                      value={editedMous[m.id]?.title ?? m.title}
                      onChange={(e) =>
                        setEditedMous((prev) => ({
                          ...prev,
                    ...
                    />
                    <textarea
                      className="text-muted-foreground leading-relaxed w-full bg-primary/5 p-1 rounded"
                      value={editedMous[m.id]?.body ?? m.body}
                      onChange={(e) =>
                        setEditedMous((prev) => ({
                          ...prev,
      ...
      <section className="py-20 bg-sand">
        <div className="container-narrow">
          <div className="flex justify-between items-end mb-12">
            <RevealOnScroll>
              <SectionLabel eyebrow="Partners" title="MOU certificates" align="center" />
            </RevealOnScroll>
            {isEditMode && (
              <button
                onClick={() => handleAddMou("certificate")}
                className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-full font-bold text-sm"
              >
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {certificateMous.map((m: any, i: number) => (
              <RevealOnScroll key={m.id} delay={i * 120}>
                <div
                  className={`group rounded-3xl overflow-hidden bg-card border transition-all relative ${isEditMode ? "border-primary/30" : "border-border shadow-[var(--shadow-card)] hover-lift"}`}
                >
                  {isEditMode && (
                    <button
                      onClick={async () => {
                      ...
                      <div className="w-full max-w-[280px] space-y-2">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditedMous((prev) => ({
                              ...prev,
                              [m.id]: { ...prev[m.id], badge: e.target.value },
                            }))
                          }
      ...
      {isEditMode && hasChanges && (
        <div className="fixed bottom-8 right-8 z-50 animate-in fade-in zoom-in slide-in-from-bottom-4">
          ...
          >
            <Save className="h-5 w-5" /> Save MOUs
          </button>
        </div>
      )}
    </>
